...
locals {
  ...
  ratePolicies = flatten([ for policy in local.settings.security.policies : [ for ratePolicy in policy.ratePolicies : { id: akamai_appsec_rate_policy.default[ratePolicy.name].rate_policy_id, name: ratePolicy.name, action: ratePolicy.action, securityPolicyId: akamai_appsec_security_policy_default_protections.default[policy.name].security_policy_id } ] ])
}
...
resource "akamai_appsec_rate_policy_action" "default" {
  ...
  ipv4_action        = each.value.action
  ipv6_action        = each.value.action
